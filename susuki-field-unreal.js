...
export function buildSusukiCtx(opts) {
  ...
  return {
    ...
    uSusukiPlumeFlex:
      uniforms.uSusukiPlumeFlex ?? uniform(params.plumeFlex ?? 0.2),
    uSusukiPlumeSoftEdge:
      uniforms.uSusukiPlumeSoftEdge ?? uniform(params.plumeSoftEdge ?? 0),
    ...
    uWindAxis:
      ...
      uniform(
        new THREE.Vector3(
          Math.sin(params.windDir ?? 0.7),
          0,
          -Math.cos(params.windDir ?? 0.7),
        ),
      ),
    uCrossAxis:
      uniforms.uCrossAxis ??
    ...
    uStemColor:
      uniforms.uSusukiStemColor ?? uniform(hexToVec3(params.stemColor)),
    uPlumeColor:
      uniforms.uSusukiPlumeColor ?? uniform(hexToVec3(params.plumeColor)),
    uSunDir:
      uniforms.uSunDir ?? uniform(new THREE.Vector3(0.5, 0.7, 0.5).normalize()),
    ...
    uSpecV2Dir:
      uniforms.uSpecV2Dir ??
      uniform(new THREE.Vector3(-1, 0.45, 1).normalize()),
    ...
    uTrailCenter:
      uniforms.uTrailCenter ?? uniform(new THREE.Vector2(9999, 9999)),
    uPlayerPos:
      uniforms.uPlayerPos ?? uniform(new THREE.Vector3(9999, 0, 9999)),
...
const STEM_VERTS_LOW = (SUSUKI_STEM_SEGMENTS_LOW + 1) * 3;
const STEM_VERTS_HIGH = (SUSUKI_STEM_SEGMENTS_HIGH + 1) * 3;
...
export function setupSusukiPatches(
  ...
  const nearAabbSize = new THREE.Vector3(
    NEAR_PATCH_SIZE,
    1000,
    NEAR_PATCH_SIZE,
  );
