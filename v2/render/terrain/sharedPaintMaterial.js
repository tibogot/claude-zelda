 ...
 */
export function createV2PaintMaterial(layerSlots, chunkSize, worldSize, cliffDeps = null) {
  if (layerSlots.length !== 4) {
    throw new Error(`createV2PaintMaterial expects 4 layer slots, got ${layerSlots.length}`);
  }

  const mat = new THREE.MeshStandardNodeMaterial({ roughness: 0.88, metalness: 0.0 });
  ...
  mat.colorNode = Fn(() => {
    ...
    const blended = c0.mul(w.x).add(c1.mul(w.y)).add(c2.mul(w.z)).add(c3.mul(w.w));
  ...
  mat.normalNode = Fn(() => {
    ...
    const nx = nx0.mul(w.x).add(nx1.mul(w.y)).add(nx2.mul(w.z)).add(nx3.mul(w.w));
    const ny = ny0.mul(w.x).add(ny1.mul(w.y)).add(ny2.mul(w.z)).add(ny3.mul(w.w));
    const nz = sqrt(max(float(0), float(1).sub(nx.mul(nx)).sub(ny.mul(ny))));
    const imgRaw = vec3(nx.mul(0.5).add(0.5), ny.mul(0.5).add(0.5), nz.mul(0.5).add(0.5));
