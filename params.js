/**
 * Single source of truth for Tweakpane / app params.
 * Export PARAMS; index.html imports and may mutate (e.g. PARAMS.terrainHeight) for createTerrain/regen.
 */
export const PARAMS = {
  grassWidth: 0.08,
  grassHeight: 0.7,
  grassBaseBend: 1.0,
  lodDistance: 28,
  maxDistance: 80,
  lodBlendStart: 0.35,
  nearRingExtent: 3,
  nearFadeRange: 8,
  grassBladesRegular: 1.0,
  grassBladesNear: 1.0,
  gradientCurve: 2.0,
  baseColor1: "#143210",
  baseColor2: "#183a12",
  tipColor1: "#a6cc40",
  tipColor2: "#cce666",
  colorVariation: 0.3,
  lushColor: "#1a4a0a",
  bleachedColor: "#d4cc80",
  windSpeed: 1.0,
  windStrength: 0.4,
  windWaveScale: 0.08,
  windDir: 0.7,
  windGust: 0.3,
  windMicroSway: 0.15,
  // Susuki (pampas grass) — shares wind with grass, same size as grass by default
  susukiEnabled: false,
  susukiStemHeight: 0.7,
  susukiStemWidth: 0.08,
  susukiBandWidth: 0.06,
  susukiPlumeStart: 0.6,
  susukiPlumeFlex: 0.2,
  susukiPlumeSoftEdge: 0,
  susukiStemColor: "#2d5a1f",
  susukiPlumeColor: "#ffffff",
  susukiLodDistance: 18,
  susukiMaxDistance: 40,
  susukiGridSize: 12,
  susukiNearRingExtent: 4,
  interactionEnabled: true,
  interactionRange: 1.8,
  interactionStrength: 0.35,
  interactionHeightThreshold: 1.5,
  interactionRepel: true,
  trailEnabled: false,
  trailCrushSpeed: 0.22,
  trailGrowRate: 0.012,
  trailRadius: 0.85,
  aoEnabled: true,
  aoIntensity: 1.0,
  aoBaseRange: 0.5,
  grassCylindricalStrength: 1.0,
  minSkyBlend: 0.55,
  maxSkyBlend: 0.7,
  bsEnabled: true,
  bsIntensity: 0.4,
  bsColor: "#51cc66",
  bsPower: 2.0,
  frontScatter: 0.3,
  rimSSS: 0.4,
  // Specular V1 (directional highlight)
  specV1Enabled: false,
  specV1Intensity: 1.5,
  specV1Color: "#ffffff",
  specV1DirX: -1.0,
  specV1DirY: 1.0,
  specV1DirZ: 0.5,
  // Specular V2 (scattered glints)
  specV2Enabled: false,
  specV2Intensity: 1.0,
  specV2Color: "#ffffff",
  specV2DirX: -1.0,
  specV2DirY: 0.45,
  specV2DirZ: 1.0,
  specV2NoiseScale: 3.0,
  specV2NoiseStr: 0.6,
  specV2Power: 12.0,
  specV2TipBias: 0.5,
  fogEnabled: true,
  fogNear: 20.0,
  fogFar: 90.0,
  fogIntensity: 0.52,
  fogColor: "#a8c0d8",
  hazeEnabled: false,
  hazeThickness: 35,
  hazeOpacity: 0.9,
  hazeFalloff: 2.2,
  hazeColor: "#f5f5f0",
  hazeNoiseScaleA: 0.004,
  hazeNoiseScaleB: 0.008,
  hazeNoiseAmountA: 0.22,
  hazeNoiseAmountB: 0.18,
  hazeNoiseDetail: 0.12,
  hazeNoiseSpeed: 1,
  seasonalEnabled: false,
  seasonalStrength: 0.3,
  seasonalScale: 0.02,
  seasonalDryColor: "#8b7d3c",
  groundVariation: true,
  groundBaseColor: "#4a9030",
  groundDirtColor: "#8b6f4a",
  grassSlopeMin: 0.55,
  grassSlopeMax: 0.92,
  grassAmount: 1.0,
  texTiling: 80.0,
  sunDirX: -1.0,
  sunDirY: 1.2,
  sunDirZ: 1.0,
  sunIntensity: 2.2,
  sceneAmbient: 1.0,
  exposure: 0.5,
  environmentIntensity: 0.45,
  skyTurbidity: 2,
  skyRayleigh: 1.5,
  skyMie: 0.005,
  skyMieG: 0.8,
  terrainHeight: 76,
  mountainStrength: 0.65,
  fieldFlatten: 0.6,
  lakeCenterX: 65,
  lakeCenterZ: -65,
  lakeRadius: 58,
  lakeDepth: 12,
  showWater: true,
  waterLevel: 12.0,
  waterUvScale: 2.7,
  waterNormalScale: 0.08,
  waterFresnelScale: 0.6,
  waterSpeed: 0.08,
  waterShininess: 400,
  waterSunColor: "#fffaf0",
  waterHighlightsGlow: 3.5,
  waterHighlightFresnelInfluence: 0.4,
  waterHighlightsSpread: 0.3,
  waterDeepColor: "#95cddc",
  waterShallowColor: "#3a7a8a",
  waterMinOpacity: 0.6,
  // Stylized water (showcase3-style): placement + appearance
  lakeHalfSize: 35,
  waterHeightOffset: 2.0,
  waterColor: "#00f2ff",
  waterStylizedDeepColor: "#006d77",
  waterWaveSpeed: 0.8,
  waterWaveHeight: 0.25,
  waterWaveDirection2: 0.7,
  waterNoiseScale: 0.2,
  waterLineThinness: 0.04,
  waterCausticLayer2Scale: 0.08,
  waterFoamWidth: 1.2,
  waterFoamDistortion: 1.5,
  waterOpacity: 0.31,
  waterFresnelPower: 3.5,
  waterFresnelStrength: 0.35,
  waterSunHighlightSize: 0.15,
  waterSunHighlightStrength: 0.5,
  waterSegments: 256,
  waterReflectionStrength: 0.65,
  playerSpeed: 3.2,
  runSpeedMultiplier: 1.95,
  crouchSpeedMultiplier: 0.5,
  rollDashDistance: 8,
  jumpSpeed: 5.5,
  gravity: 9.81,
  glideFallSpeed: 3,
  camDist: 7,
  camHeight: 0.0,
  camPitchMin: -0.2,
  camPitchMax: 1.2,
  mouseSensitivity: 0.002,
  keyTurnSpeed: 2.2,
  cameraMode: "thirdPerson",
  npcEnabled: false,
  deerEnabled: false,
  rapierDebug: false,
  showRuins: false,
  showChurch: false,
  churchX: 39,
  churchZ: 48,
  churchScale: 10,
  churchYOffset: -3,
  showPubgChurch: false,
  pubgChurchX: -80,
  pubgChurchZ: 60,
  pubgChurchYOffset: 0,
  pubgChurchScale: 0.02,
  showTrees: false,
  showScatter: false,
  showFluffyTree: false,
  showCastle: false,
  // Scatter: configurable slots. Each slot: { key, label, url, scale, count, castShadow, alphaTest, opacity }.
  // alphaTest/opacity only affect foliage (transparent) models.
  scatterSlots: [
    { key: "slot0", label: "Boulder", url: "models/rock_boulder.glb", scale: 0.04, count: 1500, castShadow: true, alphaTest: 0.05, opacity: 1 },
    { key: "slot1", label: "Game rock", url: "models/rock__game_asset.glb", scale: 0.18, count: 1000, castShadow: true, alphaTest: 0.05, opacity: 1 },
    { key: "slot2", label: "Flowers", url: "models/low_poly_flower-transformed.glb", scale: 0.15, count: 8000, castShadow: false, alphaTest: 0.05, opacity: 1 },
  ],
  scatterPresetModels: [
    "models/rock_boulder.glb",
    "models/rock__game_asset.glb",
    "models/low_poly_flower-transformed.glb",
    "models/low_poly_fern.glb",
    "models/cliff_shrub_for_terrain.glb",
  ],
  scatterScaleVariation: 0.5,
  scatterInnerRadius: 18,
  scatterCastShadow: true,
  scatterLodDistance: 50,
  scatterCulling: true,
  treeCount: 10000,
  treeCulling: true,
  treeScale: 10,
  treeAlphaTest: 0.1,
  treeOpacity: 1,
  treeDepthWrite: true,
  octahedralForestEnabled: false,
  octahedralForestScale: 1,
  octahedralForestTreeScale: 1.5,
  octahedralForestAlphaClamp: 0.15,
  octahedralForestLod0Alpha: 0.1,
  octahedralForestLodDist: 60,
  octahedralForestLod2Dist: 150,
  octahedralForestFadeRange: 8,
  octahedralForestSunColor: "#d9c799",
  octahedralForestAmbColor: "#596680",
  octahedralForestLightScale: 1.0,
  octahedralForestWindStrength: 0.10,
  octahedralForestWindSpeed: 0.2,
  octahedralForestWindDirX: 1.0,
  octahedralForestWindDirZ: 0.3,
  octahedralForestLod0Vis: true,
  octahedralForestLod1Vis: true,
  octahedralForestLod2Vis: true,
  octahedralForestWireframe: false,
  octahedralForestNearCount: 0,
  octahedralForestLod1Count: 0,
  octahedralForestLod2Count: 0,
  octahedralForestTreeCount: 2000,
  octahedralForestSprites: 8,
  octahedralForestTexSize: 2048,
  octahedralForestBakeSingle: false,
  octahedralForestModelPath: "models/tree.glb",
  birdsEnabled: true,
  birdsCount: 2048,
  birdsCenterY: 120,
  birdsMinY: 80,
  birdsMaxY: 180,
  birdsSeparation: 10,
  birdsAlignment: 22,
  birdsCohesion: 30,
  shadowBias: -0.002,
  shadowNormalBias: 0.08,
  // Folio foliage (bushes + trees) — colors and cheap realism
  foliageBushColorA: "#b4b536",
  foliageBushColorB: "#d8cf3b",
  foliageTreeColorA: "#8fa835",
  foliageTreeColorB: "#b8c94e",
  foliageAoStrength: 0.35,
  foliageSssStrength: 0.25,
  foliageSssColor: "#6b8c3a",
  foliageBushScale: 1.0,
  foliageLeafScale: 1.0,
  foliageScaleMultiplier: 1.0, // legacy alias for field.html
  foliageOrientations: 5,
  foliageColorVariation: 0.15,
  foliageConditionalDepthEnabled: false,
  foliageDepthFadeNear: 2,
  foliageDepthFadeFar: 6,
  foliageBaseContactStrength: 0.25,
  trunkBaseAoStrength: 0.5,
  foliageAlphaThreshold: 0.3,
  foliageAlphaTest: 0.1,
  foliageLeafShapeVariation: 0.08,
  // Folio showcase — lighting & sky
  folioSunColor: "#fff5e0",
  folioHemiSkyColor: "#c8e0ff",
  folioHemiGroundColor: "#88aa55",
  folioGroundColor: "#4a8c30",
  folioGroundNoiseColor: "#6b5d3a",
  folioGroundNoiseStrength: 0.15,
  folioGroundNoiseScale: 0.04,
  postProcessingEnabled: false,
  gtaoEnabled: false,
  gtaoIntensity: 1.2,
  gtaoRadius: 1.2,
  gtaoSamples: 14,
  gtaoResolutionScale: 0.5,
  gtaoScale: 1,
  gtaoDistanceFallOff: 1,
  gtaoDistanceExponent: 1,
  gtaoThickness: 1,
  gtaoDenoiseEnabled: true,
  gtaoDenoiseRadius: 5,
  gtaoDenoiseLumaPhi: 5,
  gtaoDenoiseDepthPhi: 5,
  gtaoDenoiseNormalPhi: 5,
  dofEnabled: false,
  dofFocusDistance: 25,
  dofBlurStart: 5,
  dofBlurEnd: 60,
  dofBlurSize: 1,
  dofBlurSpread: 3,
  godRaysEnabled: false,
  godRaysStrength: 0.15,
  godRaysDecay: 0.96,
  godRaysDensity: 0.8,
  godRaysSamples: 32,
  bloomEnabled: false,
  bloomStrength: 0.4,
  bloomRadius: 0.4,
  bloomThreshold: 0.6,
  lensflareEnabled: false,
  lensflareBloomThreshold: 0.72,
  lensflareThreshold: 0.6,
  lensflareGhostAttenuation: 25,
  lensflareGhostSpacing: 0.25,
  characterHeight: 2.5,
  characterOffsetY: 0,
  capsuleRadius: 0.35,
};
