...
export async function createTerrain({
  ...
  function getHeight(x, z) {
    ...
    const half = size * 0.5;
    const coastN =
      fbm(perlin, x * 0.004 + 11, 5.1, z * 0.004 - 7, 3, 0.5, 2.0) *
      ts.coastNoise;
  ...
  const triRGB = (tex, sc) => () => {
    ...
    const ax = texture(
      tex,
      mul(vec2(positionWorld.y, positionWorld.z), sc),
    ).rgb;
  ...
  try {
    ...
    const layerWeights = () => {
      ...
      const wHeightRock = mul(
        smoothstep(float(48), float(158), yW),
        float(0.12),
      );
      const wRock = min(
    ...
    const gRGB = triRGB(grass.color, uGrassScale),
      rRGB = triRGB(rock.color, uRockScale),
      sRGB = triRGB(snow.color, uSnowScale);
    const gR = triR(grass.roughness, uGrassScale),
      rR = triR(rock.roughness, uRockScale),
      sR = triR(snow.roughness, uSnowScale);
    const gAo = triR(grass.ao, uGrassScale),
      rAo = triR(rock.ao, uRockScale),
      sAo = triR(snow.ao, uSnowScale);
    ...
    material.positionNode = (() => {
      ...
      const wCliffD = pow(
        smoothstep(float(0.12), float(0.36), slopeD),
        float(0.7),
      );
    ...
    console.warn(
      "[daynight-terrain] PBR textures failed to load — matte fallback.",
      err,
    );
    material = new THREE.MeshStandardNodeMaterial({ side: THREE.FrontSide });
