...
export function applyImageSlotAlbedoAndAO(col, cs, worldSize, imgWeightTex, slots) {
  ...
  out = out.mul(mix(float(1), texture(s0.ormTex, iuv0).r, imgW.r.mul(s0.uHasAO)));
  out = mix(out, texture(s1.albedoTex, iuv1).rgb, imgW.g.mul(s1.uHasAlbedo));
  out = out.mul(mix(float(1), texture(s1.ormTex, iuv1).r, imgW.g.mul(s1.uHasAO)));
  out = mix(out, texture(s2.albedoTex, iuv2).rgb, imgW.b.mul(s2.uHasAlbedo));
  out = out.mul(mix(float(1), texture(s2.ormTex, iuv2).r, imgW.b.mul(s2.uHasAO)));
  return out;
}
...
export function applyImageSlotRoughness(baseRough, cs, worldSize, imgWeightTex, slots) {
  ...
  const iuv2 = positionWorld.xz.mul(ws).mul(s2.uUVScale);
  let r = baseRough;
  r = mix(r, texture(s0.ormTex, iuv0).g, imgW.r.mul(s0.uHasRough));
  r = mix(r, texture(s1.ormTex, iuv1).g, imgW.g.mul(s1.uHasRough));
  r = mix(r, texture(s2.ormTex, iuv2).g, imgW.b.mul(s2.uHasRough));
