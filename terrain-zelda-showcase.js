/**
 * Terrain for zelda-terrain-showcase. Exact copy of terrain-sandbox.js so we can
 * modify it without touching grass-sandbox or other pages. Original: terrain-sandbox.js.
 */
...
export function createTerrain(scene, PARAMS, options) {
  ...
  let currentHeight = TERRAIN_HEIGHT;
  function setTerrainHeight(h) {
    currentHeight = h;
  }
  ...
  function generateHeightmap() {
    ...
    const peakCenterX = 380;
    const peakCenterZ = 320;
    const peakHeight = 300;
    const peakRadius = 260;
    const peakNoiseScale = 0.016;
    const peakNoiseOctaves = 3;
    for (let y = 0; y < TERRAIN_RES; y++) {
      for (let x = 0; x < TERRAIN_RES; x++) {
        ...
        const nx = wx * 0.002;
        const nz = wz * 0.002;
        const roll = fbmNoise(nx, nz, 4);
        let h = 0.42 + roll * 0.42;
        if (flat > 0) {
          h *= 0.5 + 0.5 * (1 - flat * 0.4);
        }
        h *= H;
        const dx = wx - lx,
          dz = wz - lz;
        ...
        h -= ld * lakeSmooth;
        const pdx = wx - peakCenterX;
        const pdz = wz - peakCenterZ;
        const distToPeak = Math.sqrt(pdx * pdx + pdz * pdz);
        const tRaw = Math.max(0, 1 - distToPeak / peakRadius);
        const t = tRaw * tRaw * (3 - 2 * tRaw);
        const dome = Math.pow(t, 0.62);
        const ridge = ridgedFbm(
          pdx * peakNoiseScale,
          pdz * peakNoiseScale,
          peakNoiseOctaves,
        );
        const ridgeBlend = 0.7 + 0.3 * ridge;
        const peakShape = dome * ridgeBlend;
        h += peakHeight * peakShape;
