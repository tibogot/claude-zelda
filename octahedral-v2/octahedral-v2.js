/**
 * Octahedral Impostor Editor v2 — MeshStandardNodeMaterial pipeline.
 * Entry: octahedral-v2.html
 *
 * Key differences vs octahedral-final:
 *   • Impostor uses MeshStandardNodeMaterial — integrates with scene lights,
 *     environment, fog, tone mapping for free (no hand-rolled PBR).
 *   • Shadow casting via customDepthMaterial that mirrors the billboard +
 *     alpha-cutoff so silhouettes are correct from the sun's POV.
 *   • Translucency (back-lit SSS) on emissiveNode — readable foliage.
 *   • Wind sway baked into positionNode (base-anchored).
 *   • Runtime quality presets (low/med/high) toggle barycentric blend +
 *     parallax via uniforms — no shader recompile.
 */
...
export async function run() {
  ...
  // ── GUI ────────────────────────────────────────────────────────────────
  const gui = new GUI({ title: "Octahedral v2", width: 260 });
  gui.domElement.style.cssText = "position:fixed;top:40px;right:8px;z-index:10;";
  ...
  fModel.add(P, "model", ALL_MODELS).name("Model").onChange(async (v) => {
    if (GLB_MODELS[v]) await loadGLBPath(GLB_MODELS[v], v);
    else loadPrimitive(v);
    updateTopbar();
    await rebake();
  });
  fModel.add({ load: () => fileInput.click() }, "load").name("Load GLB…");
  fModel.add({ rebake: () => rebake() }, "rebake").name("Rebake atlas [B]");
  ...
  // Atlas — changes auto-rebake
  const fAtlas = gui.addFolder("Atlas (auto-rebake)");
  ...
  fQual.add(P, "quality", ["Low", "Medium", "High"]).name("Preset [Q]").onChange(syncParams);
  fQual.add(P, "alphaCutoff", 0.05, 0.95, 0.01).name("Alpha cutoff").onChange(syncParams);
  ...
  fTree.add(P, "translucency", 0, 1.5, 0.02).name("Back-light SSS").onChange(syncParams);
  fTree.add(P, "translucencyPower", 1, 12, 0.5).name("SSS sharpness").onChange(syncParams);
  fTree.addColor(P, "translucencyTint").name("SSS tint").onChange(syncParams);
  fTree.add(P, "windAmp", 0, 0.08, 0.002).name("Wind amplitude").onChange(syncParams);
  fTree.add(P, "windFreq", 0.2, 4, 0.05).name("Wind frequency").onChange(syncParams);

  // Sun / lighting
  const fSun = gui.addFolder("Lighting");
  fSun.add(P, "sunAzimuth", 0, 360, 1).name("Sun azimuth").onChange(syncParams);
  ...
  // Debug / view
  const fDbg = gui.addFolder("Debug / view");
  fDbg.add(P, "debugMode", { "Full PBR": 0, "Normals [N]": 1, "Raw atlas [R]": 2 })
      .name("Display").onChange(syncParams);
  fDbg.add(P, "freeze").name("Freeze angle [F]").onChange(() => {
    if (P.freeze && impUniforms) {
      const camLocal = new THREE.Vector3()
        .subVectors(camera.position, impUniforms.uCenter.value)
        .divideScalar(impUniforms.uScale.value);
  ...
  fDbg.add(P, "autoOrbit").name("Auto-orbit [O]").onChange(syncParams);
  fDbg.add(P, "showAtlas").name("Show atlas dock").onChange(syncParams);
  fDbg.add(P, "showGrid").name("Dock: grid lines").onChange(() => drawAtlasDock());
  fDbg.add(P, "showActive").name("Dock: active cells").onChange(() => drawAtlasDock());

  // Camera presets
  const fCam = gui.addFolder("Camera");
  for (const name of Object.keys(CAM_PRESETS)) {
    fCam.add({ go: () => applyCameraPreset(name) }, "go").name(name);
  }

  // Export
  const fExp = gui.addFolder("Export");
  const mkExport = (key, label) => fExp.add({
    fn: () => {
  ...
  addEventListener("keydown", (e) => {
    ...
    switch (e.code) {
      case "KeyN": P.debugMode = P.debugMode === 1 ? 0 : 1; syncParams();
                   gui.controllersRecursive().forEach((c) => c.updateDisplay()); break;
      case "KeyR": P.debugMode = P.debugMode === 2 ? 0 : 2; syncParams();
                   gui.controllersRecursive().forEach((c) => c.updateDisplay()); break;
      case "KeyF": toggleFreeze(); break;
      case "KeyO": P.autoOrbit = !P.autoOrbit; syncParams();
                   gui.controllersRecursive().forEach((c) => c.updateDisplay()); break;
      case "KeyB": rebake(); break;
      case "KeyQ": cycleQuality(); break;
      case "KeyH": {
        uiHidden = !uiHidden;
        const elements = [gui.domElement, dock, $("stats"), $("keys"), $("topbar"), $("lbl-ref"), $("lbl-imp"), $("divider")];
        for (const el of elements) if (el) el.style.display = uiHidden ? "none" : "";
        if (!uiHidden && dock) dock.style.display = P.showAtlas ? "flex" : "none";
        break;
      }
