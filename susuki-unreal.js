...
export function createSusukiBandMaterial(segments, verts, ctx) {
  ...
  const positionNode = Fn(() => {
    ...
    const stemTipWorld = add(plantOffset, stemGrassMat.mul(stemTipLocal));
    const totalWindLean = mul(add(windLean, micro), heightPct);
    ...
    const gvn1 = mul(bandMat, rotateY_mat(mul(PI, 0.3, zSide)).mul(gvn)).mul(
      zSide,
    );
    const gvn2 = mul(bandMat, rotateY_mat(mul(PI, -0.3, zSide)).mul(gvn)).mul(
      zSide,
    );
