/**
 * Player character for parkourtsushima: uses UA1+UA2_compressed.glb (same rig, more animations).
 * Otherwise identical to player.js — capsule, camera, input, movement, animation logic unchanged.
...
const CHAR_GLB = "models/UA1+UA2_compressed.glb";
...
export function createPlayer(opts) {
  ...
  charLoader.load(
    ...
    (gltf) => {
      ...
      const HAND_BONE_R = "hand_r";
      const HAND_BONE_L = "hand_l";
      const HEAD_BONE_NAMES = ["Head", "neck_01"];
      ...
      // Attach katana.glb to the right hand (replaces procedural sword)
      const rightHand = characterGroup.userData.rightHandBone;
      if (rightHand) {
        const swordGroup = new THREE.Group();
        // Position and rotation: fine-tuned so blade sits in hand (match parkourParams.katana defaults)
        ...
        const loadKatana = () => {
          ...
          (katanaGltf) => {
            ...
            // Scale and origin: target total length ~1.0 (like procedural sword), grip at origin for easy hand placement
            const box = new THREE.Box3().setFromObject(katanaScene);
            const size = new THREE.Vector3();
            box.getSize(size);
            const length = Math.max(size.x, size.y, size.z);
            const targetLength = 1.0;
            const scale = targetLength / (length || 1);
            katanaScene.scale.setScalar(scale);
            box.setFromObject(katanaScene);
            // Put grip end (bbox min corner) at group origin so we can place that point in the hand
            katanaScene.position.set(-box.min.x, -box.min.y, -box.min.z);
            swordGroup.add(katanaScene);
          },
          undefined,
          (err) => {
            console.warn("Katana GLB load failed, using procedural sword:", err);
            const handleGeo = new THREE.CylinderGeometry(0.02, 0.025, 0.18, 8);
            const bladeGeo = new THREE.BoxGeometry(0.015, 0.06, 0.95);
            const handleMat = new THREE.MeshStandardNodeMaterial({ color: 0x4a3728, roughness: 0.8, metalness: 0.1 });
            const bladeMat = new THREE.MeshStandardNodeMaterial({ color: 0xc0c8d0, roughness: 0.3, metalness: 0.8 });
        ...
        const ktx2Promise = ktx2Loader.detectSupport(renderer);
        if (ktx2Promise && typeof ktx2Promise.then === "function") {
          ktx2Promise.then(loadKatana).catch((e) => {
            console.warn("KTX2 detectSupport failed, loading katana without KTX2:", e);
            loadKatana();
          });
        } else {
          loadKatana();
      ...
      // Find foot bones for footstep sync (foot_l / foot_r in UA1+UA2)
      let _fbl = null,
        _fbr = null;
      model.traverse((o) => {
        const n = o.name;
      ...
      try {
        if (gltf.animations && gltf.animations.length) {
          ...
          // UA1+UA2_compressed: clip names use _Armature suffix (e.g. Idle_Loop_Armature)
          const idleClip =
            gltf.animations.find((a) => a.name === "Idle_Loop_Armature") ||
            gltf.animations.find((a) => a.name === "Idle_Loop") ||
            gltf.animations[0];
          const walkClip =
            gltf.animations.find((a) => a.name === "Walk_Loop_Armature") ||
            gltf.animations.find((a) => a.name === "Walk_Loop") ||
            gltf.animations[0];
          const runClip =
            gltf.animations.find((a) => a.name === "Sprint_Loop_Armature") ||
            gltf.animations.find((a) => a.name === "Jog_Fwd_Loop_Armature") ||
          ...
          const crouchWalkClip =
            gltf.animations.find((a) => a.name === "Crouch_Fwd_Loop_Armature") ||
            gltf.animations.find((a) => a.name === "Crouch_Fwd_Loop") ||
            crouchClip;
          const rollClip =
            gltf.animations.find((a) => a.name === "Roll_Armature") ||
            gltf.animations.find((a) => a.name === "Roll_RM_Armature") ||
